...
#define PORT    65022      // 임의로 설정한 통신포트

int stored_student_ID[MAX_STUDENT_COUNT] = {201421092, 201320206, 201521061, 201521086};
...
char student_arr[MAX_STUDENT_COUNT][19] = {0,};
...
int uid_to_ID(int uid){
	for(int i = 0; i<MAX_STUDENT_COUNT; i++){
		if(stored_student_arr_int[i]==uid){
			return stored_student_ID[i];
		}
	}
	return 0;
}
        ...
        while(1){
			...
			if(int_uid != 0){
				...
				if(uid_to_name(int_uid) == 0){
					...
					printf("What your Student ID : \n");
					scanf("%d", &new_student_ID);
					stored_student_ID[uid_stored_student_count] = new_student_ID;
				...
				student_arr[class_in_student_count][15] = uid_to_ID(int_uid)>>24;
				student_arr[class_in_student_count][16] = uid_to_ID(int_uid)>>16;
				student_arr[class_in_student_count][17] = uid_to_ID(int_uid)>>8;
				student_arr[class_in_student_count][18] = uid_to_ID(int_uid);
            ...
            if(strncmp(student_arr[class_in_student_count], "exit", 4) ==0) {        //입력 문자가 "exit" 로 시작하면 -> exit 콜 =>종료
				...
				for(int i =0; i<MAX_STUDENT_COUNT; i++){
					...
					printf(" %d", (student_arr[i][15]<<24)+ (student_arr[i][16]<<16) + (student_arr[i][17]<<8)+ (student_arr[i][18]));
					printf("\n");
				}
			}
